...
func (n *ImapOpts) getTicketMasterMFA() (string, error) {
	// connect to server
	...
	for _, box := range boxes {
		...
		for msg := range messages {
			...
			if !strings.Contains(strings.ToLower(fromaddress), "ticketmaster") {
				continue
			}
			...
			for {
				...
				case *mail.InlineHeader:
					...
					doc.Find("td").Each(func(_ int, tag *goquery.Selection) {
						codeSrc, _ := tag.Html()
						if strings.Contains(codeSrc, "Didn’t Request a Code?") {
							doc, _ := goquery.NewDocumentFromReader(strings.NewReader(codeSrc))
							doc.Find("td").Each(func(_ int, tag *goquery.Selection) {
								code, _ := tag.Html()
								class, _ := tag.Attr("class")
								if class == "innertd buttonblock" {
